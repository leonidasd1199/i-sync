/**
 * Updates MANIFIESTO_CARGA document_templates to the canonical operational layout.
 */
const MANIFIESTO_CARGA_STANDARD_HTML = `
...
const MANIFIESTO_CARGA_STANDARD_CSS = `
...
module.exports = {
  async up(db) {
    ...
    const result = await collection.updateMany(
      ...
      {
        $set: {
          html: MANIFIESTO_CARGA_STANDARD_HTML.trim(),
          css: MANIFIESTO_CARGA_STANDARD_CSS.trim(),
          updatedAt: new Date(),
        },
    ...
    console.log(
      `✅ MANIFIESTO_CARGA template upgrade: matched ${result.matchedCount}, modified ${result.modifiedCount}`,
    );
  },

  async down() {
    console.warn(
      "⚠️  20260330120000-upgrade-manifiesto-template-layout: no automatic down — restore templates from backup or re-seed.",
    );
  },
};
