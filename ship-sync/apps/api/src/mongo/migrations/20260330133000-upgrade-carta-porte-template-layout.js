/**
 * Updates CARTA_PORTE document_templates to canonical operational layout.
 */
const CARTA_PORTE_STANDARD_HTML = `
...
const CARTA_PORTE_STANDARD_CSS = `
...
module.exports = {
  async up(db) {
    ...
    const result = await collection.updateMany(
      ...
      {
        $set: {
          html: CARTA_PORTE_STANDARD_HTML.trim(),
          css: CARTA_PORTE_STANDARD_CSS.trim(),
          updatedAt: new Date(),
        },
    ...
    console.log(
      `✅ CARTA_PORTE template upgrade: matched ${result.matchedCount}, modified ${result.modifiedCount}`,
    );
  },

  async down() {
    console.warn(
      "⚠️  20260330133000-upgrade-carta-porte-template-layout: no automatic down — restore templates from backup or re-seed.",
    );
  },
};
