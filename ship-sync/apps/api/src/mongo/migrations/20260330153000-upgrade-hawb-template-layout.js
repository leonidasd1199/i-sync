/**
 * Updates HAWB document_templates to canonical operational layout.
 */
const HAWB_STANDARD_HTML = `
...
module.exports = {
  async up(db) {
    ...
    const result = await collection.updateMany(
      ...
      {
        $set: {
          html: HAWB_STANDARD_HTML.trim(),
          css: HAWB_STANDARD_CSS.trim(),
          updatedAt: new Date(),
        },
    ...
    console.log(
      `✅ HAWB template upgrade: matched ${result.matchedCount}, modified ${result.modifiedCount}`,
    );
  },

  async down() {
    console.warn(
      "⚠️  20260330153000-upgrade-hawb-template-layout: no automatic down — restore templates from backup or re-seed.",
    );
  },
};
