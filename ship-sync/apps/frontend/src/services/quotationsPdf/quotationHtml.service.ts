...
export const buildQuotationHTML = (quotation: any): string => {
        ...
        if (usesPricelistItems) {
            ...
            for (const route of sl.routes) {
                for (const item of route.pricelistItems ?? []) {
                    if (!seen.has(item.chargeType)) {
                        seen.add(item.chargeType);
                        pricelistRows.push({
            ...
            rowsHTML = pricelistRows.map(({ chargeType, label }, rowIdx) => `
<tr style="${rowIdx % 2 === 0 ? "" : "background:#F9FAFB;"}">
<td>${label}</td>
${sl.routes.map((r: any) => {

                const item = r.pricelistItems?.find(
                    (i: any) => i.chargeType === chargeType
                );

                if (!item) return `<td>—</td>`;

                const withProfit = calcWithProfit(item.cost, item.profit);
